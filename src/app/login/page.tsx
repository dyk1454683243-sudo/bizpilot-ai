...
export default function LoginPage() {
  ...
  const features = [
    ...
    { icon: <CheckCircle className="h-5 w-5" />, text: 'Automated appointment reminders' },
    { icon: <Zap className="h-5 w-5" />, text: 'One-click invoice generation' },
  ];
      ...
      <div className="hidden lg:flex lg:w-1/2 xl:w-[55%] gradient-hero relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-center px-12 xl:px-20 py-12 w-full">
          ...
          <h1 className="text-4xl xl:text-5xl font-extrabold text-white leading-tight mb-4">
            ...
            <br />
            <span className="text-indigo-200">Sales & Operations</span>
          </h1>
          <p className="text-lg text-indigo-200 mb-10 max-w-md">
            Never lose a customer again. Automate follow-ups, bookings, invoices, and get AI-powered insights.
          </p>
          ...
          <div className="mt-14 flex items-center gap-4">
            <div className="flex -space-x-2">
              {['RS', 'PM', 'AK', 'SG'].map((initials, i) => (
                <div
                  key={i}
                  className="w-8 h-8 rounded-full bg-white/20 backdrop-blur border-2 border-indigo-600 flex items-center justify-center text-xs font-medium text-white"
                >
                  {initials}
                </div>
              ))}
            ...
            <p className="text-sm text-indigo-200">
              <span className="font-semibold text-white">2,500+</span> businesses trust BizPilot AI
            </p>
          </div>
        </div>
      </div>
